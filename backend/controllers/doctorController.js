...
const appointmentComplete = async (req, res) => {
  try {
    ...
    if (appointmentData && appointmentData.docId.toString() === docId) {
      ...
      const feedbackLink = `${frontendUrl}/feedback/${appointmentId}`;
      const emailHTML = completionTemplate(
        userData.name,
        docData.name,
        feedbackLink,
      );
...
const consultationComplete = async (req, res) => {
  try {
    ...
    const { userData, docData } = consultationData;

    // 📧 USER EMAIL (feedback)
    const frontendUrl = process.env.FRONTEND_URL || "http://localhost:5173";
    const feedbackLink = `${frontendUrl}/consultation-feedback/${consultationId}`;

    await sendEmail(
      userData.email,
      "Consultation Completed - DocPulse",
      completionTemplate(userData.name, docData.name, feedbackLink),
    );
    ...
    await sendEmail(
      ...
      `Your consultation with ${userData.name} has been completed successfully.`,
    );
