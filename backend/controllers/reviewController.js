...
const getTopReviews = async (req, res) => {
    try {
        ...
        const formattedReviews = reviews.map(rev => ({
            ...
            role: "Patient",
            image: rev.userId?.image || "https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png",
            rating: rev.rating,
            text: rev.summary
        }));
...
const getDoctorReviews = async (req, res) => {
    try {
        ...
        res.json({ success: true, reviews: formattedReviews, averageRating: Number(averageRating) });
    } catch (error) {
