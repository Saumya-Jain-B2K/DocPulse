  ...
  cron.schedule("* * * * *", async () => {
    try {
      ...
      for (const c of consultations) {
        const consultationDateTime = new Date(`${c.slotDate} ${c.slotTime}`);

        if (consultationDateTime <= now) {
          const link = `${process.env.FRONTEND_URL}/consultation/chat/${c.chatRoomId}`;

          await sendEmail(c.userData.email, "Consultation Started", link);
          await sendEmail(c.docData.email, "Consultation Started", link);
