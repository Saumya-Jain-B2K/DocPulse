  ...
  useEffect(() => {
    ...
    socketRef.current.on("ai_response", (data) => {
      ...
      if (data && data.message) {
        setMessages((prev) => [
          ...prev,
          { role: "ai", content: data.message, doctors: data.doctors },
        ]);
      }
    });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <button
          ...
          className="bg-[#000B6D] text-white px-8 py-3 rounded-full"
        >
          Go to Login
        </button>
      </div>
    ...
    <div className="max-w-4xl mx-auto my-8 border rounded-lg shadow-sm bg-white overflow-hidden flex flex-col h-[85vh]">
      ...
      <div className="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col gap-4">
        {messages.map((msg, idx) => (
          ...
          >
            <div
              className={`max-w-[75%] p-4 rounded-xl shadow-sm text-sm ${msg.role === "user" ? "bg-[#000B6D] text-white rounded-tr-none" : "bg-white border text-gray-800 rounded-tl-none"}`}
            >
              {msg.role === "ai" ? (
                <>
                  ...
                  {msg.doctors && msg.doctors.length > 0 && (
                    <div className="mt-4 flex flex-col gap-2 border-t pt-3">
                      <p className="font-semibold text-sm text-[#000B6D]">
                        Recommended Specialists:
                      </p>
                      {msg.doctors.map((doc, dIdx) => (
                        <div
                          key={dIdx}
                          className="flex items-center justify-between bg-blue-50 border border-blue-100 p-3 rounded-lg shadow-sm mt-1"
                        >
                          <div>
