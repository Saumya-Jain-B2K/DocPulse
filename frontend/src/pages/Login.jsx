  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (state === "Sign Up") {
        ...
        //   email,
        // });
        // console.log("LOGIN RESPONSE:", data)  // ✅ yahan add karo
        // if (data.success) {
        //   setToken(true);
        //   // 🔥 adding this line to make the code error free
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log("Google Login UserId:", data.userData._id);
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex items-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96border rounded-xl text-zinc-600 text-m shadow-lg">
        ...
        <button
          type="submit"
          className="bg-[#000B6D] text-white w-full py-2 rounded-md text-base"
        >
          {state === "Sign Up" ? "Create Account" : "Login"}
        </button>
        ...
        {state === "Sign Up" ? (
          <p>
            ...
            <span
              ...
              className="text-[#000B6D] underline cursor-pointer"
            >
              Login here
            </span>
          </p>
