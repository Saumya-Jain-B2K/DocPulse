...
const Testimonials = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (scrollRef.current && scrollRef.current.firstChild) {
        ...
        scrollRef.current.scrollBy({ left: cardWidth * cardsPerView, behavior: 'smooth' });
      }
    }, 4000);
  ...
  return (
    <div className="py-24 text-center">
      <h1 className="text-3xl sm:text-4xl font-semibold text-gray-900">Patient Testimonials</h1>
      <p className="text-sm sm:text-base mt-3 mb-12 text-gray-500 max-w-lg mx-auto">
        Read what our patients have to say about their experience with our verified top-rated doctors.
      </p>
            ...
            >
              ...
              <div className="flex gap-1 mb-6">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span key={i} className={`text-xl leading-none ${i < item.rating ? 'text-yellow-400' : 'text-gray-200'}`}>★</span>
                ))}
              </div>
              ...
              <div className="flex items-center gap-4 mt-auto">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{item.role || 'Patient'}</p>
                </div>
              </div>
            </div>
          ))}
