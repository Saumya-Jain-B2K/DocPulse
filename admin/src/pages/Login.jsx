  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (state === "Admin") {
        ...
        if (data.success) {
          localStorage.setItem("dToken", data.token); // new line added
    ...
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex items-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-h-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg">
        <p className="text-2xl font-semibold m-auto">
          <span className="text-[#000B6D]"> {state} </span> Login
        </p>
        ...
        <button className="bg-[#000B6D] text-white w-full py-2 rounded-md text-base hover:bg-[#000b6dee] transition-all">
          Login
        </button>
