...
const DoctorProfile = () => {
  const {dToken, profileData, setProfileData, getProfileData, backendUrl} = useContext(DoctosContext)
  ...
  return profileData && (
    <div>
      <div className='flex flex-col gap-4 m-5'>
        <div>
          <img className='bg-[#000B6D]/80 w-full sm:max-w-64 rounded-lg' src={profileData.image} alt="" />
        </div>
        <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
          ...
          {
            isEdit 
            ? <button onClick={updateProfile} className='px-4 py-1 border border-[#000B6D] text-sm rounded-full mt-5 hover:bg-[#000B6D] hover:text-white transition-all'>Save</button>
            : <button onClick={() => setIsEdit(true)} className='px-4 py-1 border border-[#000B6D] text-sm rounded-full mt-5 hover:bg-[#000B6D] hover:text-white transition-all'>Edit</button>
          }
