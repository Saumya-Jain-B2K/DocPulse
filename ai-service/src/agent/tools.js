...
const recommendDoctorTool = tool(
  async ({ speciality }) => {
    try {
      ...
      const matchedDoctors = allDoctors.filter(
        (doc) => doc.speciality === speciality && doc.available === true,
      );
      ...
      console.error(
        "[TOOLS] Error fetching doctors from internal API:",
        error.message,
      );
  ...
  },
  {
    ...
    description:
      "Use this tool ONLY when you have diagnosed the user's issue and are ready to recommend exactly ONE medical speciality from the list. It will verify availability and return real doctor details.",
    schema: z.object({
      speciality: z
        .enum(validSpecialities)
        .describe("The medical speciality to recommend."),
    }),
  },
);
